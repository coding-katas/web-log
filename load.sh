...
# load index template
if curl -f -XPUT "$url/_index_template/web-logs" -H 'Content-Type: application/json' -d "@web-logs-template.json"
...
if curl -f -XPUT "$url/_ingest/pipeline/web-logs" -H 'Content-Type: application/json' -d "@web-logs-pipeline.json"
